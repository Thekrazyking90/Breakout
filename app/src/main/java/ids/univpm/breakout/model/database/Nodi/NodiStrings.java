package ids.univpm.breakout.model.database.Nodi;

public class NodiStrings {
    public static final String TBL_NAME="Nodi";
    public static final String FIELD_ID="ID_nodo";
    public static final String FIELD_ID_PIANO ="ID_piano";
    public static final String FIELD_CODE="codice";
    public static final String FIELD_COORD_Y="coord_Y";
    public static final String FIELD_COORD_X="coord_X";
    public static final String FIELD_WIDTH="larghezza";

}
